...
export const AIAnalysisModal: React.FC<AIAnalysisModalProps> = ({
  ...
  const handleAnalysis = async () => {
    ...
    if (!navigator.onLine) {
      setError('No hay conexión a internet. Por favor, verifica tu red.');
      return;
    }
    ...
    try {
      ...
      const result = await Promise.race([
        aiAnalysisService.analyzeProfessor(professor),
        timeoutPromise
      ]);
      ...
      if (err instanceof Error) {
        if (err.message === 'NO_API_KEY') {
          ...
          setError('El análisis está tardando demasiado. La IA podría estar saturada.');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-4xl max-h-[90vh] overflow-y-auto sm:mx-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 break-anywhere">
            <Bot className="h-5 w-5" />
            <span>Análisis de IA - {professor.nombre}</span>
          </DialogTitle>
        </DialogHeader>
          ...
          <Card className="p-4 bg-card">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="min-w-0">
                <h3 className="font-semibold text-lg break-anywhere">{professor.nombre}</h3>
                <p className="text-sm text-muted-foreground break-anywhere">{professor.universidad}</p>
              </div>
          ...
          {!analysis && !isLoading && (
            <div className="text-center">
              <Button 
                ...
                className="gap-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:opacity-90 w-full sm:w-auto"
                size="lg"
              >
              ...
              <Card className="p-6 bg-card">
                ...
                <ul className="space-y-3">
                  {analysis.recommendations.map((recommendation, index) => (
                    <li key={index} className="flex items-start gap-3 p-3 rounded-lg bg-yellow-100/50 dark:bg-yellow-900/20 border border-yellow-200/50 dark:border-yellow-800/30">
